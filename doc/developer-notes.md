...
- [Development guidelines](#development-guidelines)
    - [General dogxcoin Core](#general-dogxcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
dogxcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
dogxcoin Core is a multi-threaded application, and deadlocks or other
...
To profile a running dogxcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep dogxcoind` -- sleep 60
```
...
dogxcoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as dogxcoin Core, where everyone uses
...
pay attention to for reviewers of dogxcoin Core code.

General dogxcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: dogxcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `dogxCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef dogxCOIN_FOO_BAR_H
#define dogxCOIN_FOO_BAR_H
...
#endif // dogxCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of dogxcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against dogxcoin Core so that they can be integrated
...
- src/univalue
  - Upstream at https://github.com/dogxcoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors it will cause problems with dogxcoin's `select()` loop, because
...
compatibility between nodes. This happened in dogxcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to dogxcoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of dogxcoin Core, the wallet was always
