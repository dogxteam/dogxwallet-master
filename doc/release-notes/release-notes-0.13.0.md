dogxcoin Core version 0.13.0 is now available from:
...
Various code modernizations have been done. The dogxcoin Core code base has
...
HD wallets are incompatible with older versions of dogxcoin Core.
...
proposed parameters, a future release of dogxcoin Core is expected that
...
The two phases are distinct in the dogxcoin-Qt GUI. During the first one,
...
- #8207 `f7a403b` Add a link to the dogxcoin-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [dogxcoin-Tx] fix missing test fixtures, fix 32dogx atoi issue (jonasschnelli)
