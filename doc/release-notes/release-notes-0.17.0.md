dogxcoin Core version 0.17.0 is now available from:
...
installer (on Windows) or just copy over `/Applications/dogxcoin-Qt` (on Mac)
...
dogxcoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer (Windows XP is not supported).

dogxcoin Core should also work on most other Unix-like systems but is not
...
BIP 174 Partially Signed dogxcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/dogxcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for dogxcoin transactions that are not fully signed
...
Overall, the construction of a fully signed dogxcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid dogxcoin transaction (in network format)
...
- #13528 `b877c39` Move dogxcoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use dogxcoinTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use dogxcoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use dogxcoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use dogxcoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for dogxcoinTestFramework (WillAyd)
