...
Below are some notes on how to build dogxcoin Core for Windows.

The options known to work for building dogxcoin Core on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Bionic 18.04 is required
and is the platform used to build the dogxcoin Core Windows release binaries.
...
Note that for WSL the dogxcoin Core source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the dogxcoin Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the dogxcoin Core source code).
