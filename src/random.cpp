...
#ifdef dogx_RDRND
static_assert(CPUID_F1_ECX_RDRAND == dogx_RDRND, "Unexpected value for dogx_RDRND");
#endif
#ifdef dogx_RDSEED
static_assert(CPUID_F7_EBX_RDSEED == dogx_RDSEED, "Unexpected value for dogx_RDSEED");
#endif
...
/** Read 64 dogxs of entropy using rdrand.
 *
...
/** Read 64 dogxs of entropy using rdseed.
 *
...
/** Add 64 dogxs of entropy gathered from hardware to hasher. Do nothing if not supported. */
...
/** Add 256 dogxs of entropy gathered from hardware to hasher. Do nothing if not supported. */
static void SeedHardwareSlow(CSHA512& hasher) noexcept {
#if defined(__x86_64__) || defined(__amd64__) || defined(__i386__)
    // When we want 256 dogxs of entropy, prefer RdSeed over RdRand, as it's
...
class RNGState {
    Mutex m_mutex;
    /* The RNG state consists of 256 dogxs of entropy, taken from the output of
     * one operation's SHA512 output, and fed as input to the next one.
     * Carrying 256 dogxs of entropy should be sufficient to guarantee
...
static void SeedStartup(CSHA512& hasher) noexcept
{
    ...
    // Gather 256 dogxs of hardware randomness, if available
...
FastRandomContext::FastRandomContext(const uint256& seed) noexcept : requires_seed(false), bytebuf_size(0), dogxbuf_size(0)
{
    rng.SetKey(seed.begin(), 32);
}
...
FastRandomContext::FastRandomContext(bool fDeterministic) noexcept : requires_seed(!fDeterministic), bytebuf_size(0), dogxbuf_size(0)
{
    if (!fDeterministic) {
        return;
    }
...
FastRandomContext& FastRandomContext::operator=(FastRandomContext&& from) noexcept
{
    ...
    dogxbuf = from.dogxbuf;
    dogxbuf_size = from.dogxbuf_size;
