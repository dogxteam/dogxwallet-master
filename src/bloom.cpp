// Copyright (c) 2012-2018 The dogxcoin Core developers
...
CRollingBloomFilter::CRollingBloomFilter(const unsigned int nElements, const double fpRate)
{
    ...
    /* The maximum fpRate = pow(1.0 - exp(-nHashFuncs * nMaxElements / nFilterdogxs), nHashFuncs)
     * =>          pow(fpRate, 1.0 / nHashFuncs) = 1.0 - exp(-nHashFuncs * nMaxElements / nFilterdogxs)
     * =>          1.0 - pow(fpRate, 1.0 / nHashFuncs) = exp(-nHashFuncs * nMaxElements / nFilterdogxs)
     * =>          log(1.0 - pow(fpRate, 1.0 / nHashFuncs)) = -nHashFuncs * nMaxElements / nFilterdogxs
     * =>          nFilterdogxs = -nHashFuncs * nMaxElements / log(1.0 - pow(fpRate, 1.0 / nHashFuncs))
     * =>          nFilterdogxs = -nHashFuncs * nMaxElements / log(1.0 - exp(logFpRate / nHashFuncs))
     */
    uint32_t nFilterdogxs = (uint32_t)ceil(-1.0 * nHashFuncs * nMaxElements / log(1.0 - exp(logFpRate / nHashFuncs)));
    data.clear();
    ...
    data.resize(((nFilterdogxs + 63) / 64) << 1);
    reset();
}
