 ...
 */
static const char* dogxCOIN_PID_FILENAME = "dogxcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", dogxCOIN_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", dogxCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", dogxCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
/** Sanity checks
 *  Ensure that dogxcoin is running in a usable environment with all
...
{
    // Make sure only a single dogxcoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", dogxCOIN_CONF_FILENAME));
