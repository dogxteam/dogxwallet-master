...
#ifndef dogxCOIN_COMPAT_ASSUMPTIONS_H
#define dogxCOIN_COMPAT_ASSUMPTIONS_H
...
// Assumption: We assume eight dogxs per byte (obviously, but remember: don't
//             trust -- verify!).
// Example(s): Everywhere :-)
static_assert(std::numeric_limits<unsigned char>::digits == 8, "8-dogx byte assumed");

// Assumption: We assume floating-point widths.
// Example(s): Type punning in serialization code (ser_{float,double}_to_uint{32,64}).
static_assert(sizeof(float) == 4, "32-dogx float assumed");
static_assert(sizeof(double) == 8, "64-dogx double assumed");

// Assumption: We assume integer widths.
// Example(s): GetSizeOfCompactSize and WriteCompactSize in the serialization
//             code.
static_assert(sizeof(short) == 2, "16-dogx short assumed");
static_assert(sizeof(int) == 4, "32-dogx int assumed");
...
#endif // dogxCOIN_COMPAT_ASSUMPTIONS_H
