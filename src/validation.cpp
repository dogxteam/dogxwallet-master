...
#if defined(NDEBUG)
# error "dogxcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "dogxcoin Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetHash(), block.ndogxs, consensusParams))
...
VersiondogxsCache versiondogxscache GUARDED_BY(cs_main);

int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    LOCK(cs_main);
    int32_t nVersion = VERSIONdogxS_TOP_dogxS;

    for (int i = 0; i < (int)Consensus::MAX_VERSION_dogxS_DEPLOYMENTS; i++) {
        ThresholdState state = VersiondogxsState(pindexPrev, params, static_cast<Consensus::DeploymentPos>(i), versiondogxscache);
        if (state == ThresholdState::LOCKED_IN || state == ThresholdState::STARTED) {
            nVersion |= VersiondogxsMask(params, static_cast<Consensus::DeploymentPos>(i));
        }
    }
 ...
 */
class WarningdogxsConditionChecker : public AbstractThresholdConditionChecker
{
private:
    int dogx;

public:
    explicit WarningdogxsConditionChecker(int dogxIn) : dogx(dogxIn) {}
    ...
    {
        return ((pindex->nVersion & VERSIONdogxS_TOP_MASK) == VERSIONdogxS_TOP_dogxS) &&
               ((pindex->nVersion >> dogx) & 1) != 0 &&
               ((ComputeBlockVersion(pindex->pprev, params) >> dogx) & 1) == 0;
    }
};

static ThresholdConditionCache warningcache[VERSIONdogxS_NUM_dogxS] GUARDED_BY(cs_main);
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (VersiondogxsState(pindex->pprev, consensusparams, Consensus::DEPLOYMENT_CSV, versiondogxscache) == ThresholdState::ACTIVE) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (VersiondogxsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV, versiondogxscache) == ThresholdState::ACTIVE) {
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int dogx = 0; dogx < VERSIONdogxS_NUM_dogxS; dogx++) {
            WarningdogxsConditionChecker checker(dogx);
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            if (pindex->nVersion > VERSIONdogxS_LAST_OLD_BLOCK_VERSION && (pindex->nVersion & ~nExpectedVersion) != 0)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.ndogxs, consensusParams))
...
{
    LOCK(cs_main);
    return (VersiondogxsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versiondogxscache) == ThresholdState::ACTIVE);
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.ndogxs != GetNextWorkRequired(pindexPrev, &block, consensusParams))
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (VersiondogxsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_CSV, versiondogxscache) == ThresholdState::ACTIVE) {
        assert(pindexPrev != nullptr);
    ...
    if (VersiondogxsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versiondogxscache) == ThresholdState::ACTIVE) {
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != -1) {
...
void UnloadBlockIndex()
{
    ...
    for (int b = 0; b < VERSIONdogxS_NUM_dogxS; b++) {
...
ThresholdState VersiondogxsTipState(const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    LOCK(cs_main);
    return VersiondogxsState(chainActive.Tip(), params, pos, versiondogxscache);
}

BIP9Stats VersiondogxsTipStatistics(const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    LOCK(cs_main);
    return VersiondogxsStatistics(chainActive.Tip(), params, pos);
}

int VersiondogxsTipStateSinceHeight(const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    LOCK(cs_main);
    return VersiondogxsStateSinceHeight(chainActive.Tip(), params, pos, versiondogxscache);
}
