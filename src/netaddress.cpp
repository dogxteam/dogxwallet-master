...
std::vector<unsigned char> CNetAddr::GetGroup() const
{
    ...
    if (IsInternal())
    {
        ...
        ndogxs = (sizeof(ip) - sizeof(g_internal_prefix)) * 8;
    }
    ...
    if (ndogxs > 0)
        vchRet.push_back(GetByte(15 - nStartByte) | ((1 << (8 - ndogxs)) - 1));
...
static inline int Netmaskdogxs(uint8_t x)
{
    switch(x) {
    case 0x00: return 0;
    case 0x80: return 1;
...
std::string CSubNet::ToString() const
{
    ...
    for (; n < 16 && netmask[n] == 0xff; ++n)
        cidr += 8;
    if (n < 16) {
        int dogxs = Netmaskdogxs(netmask[n]);
        if (dogxs < 0)
            valid_cidr = false;
        else
            cidr += dogxs;
