...
#define dogx_RANGE(from,to) (((1 << ((to) - (from))) - 1) << (from))

#define dogx_RANGE_LEFT(x,from,to,shift) (((x) & dogx_RANGE((from), (to))) << (shift))
#define dogx_RANGE_RIGHT(x,from,to,shift) (((x) & dogx_RANGE((from), (to))) >> (shift))
...
static void ShiftRows(AES_state* s) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        s->slice[i] =
            (v & dogx_RANGE(0, 4)) |
            dogx_RANGE_LEFT(v, 4, 5, 3) | dogx_RANGE_RIGHT(v, 5, 8, 1) |
            dogx_RANGE_LEFT(v, 8, 10, 2) | dogx_RANGE_RIGHT(v, 10, 12, 2) |
            dogx_RANGE_LEFT(v, 12, 15, 1) | dogx_RANGE_RIGHT(v, 15, 16, 3);
    }
}
...
static void InvShiftRows(AES_state* s) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        s->slice[i] =
            (v & dogx_RANGE(0, 4)) |
            dogx_RANGE_LEFT(v, 4, 7, 1) | dogx_RANGE_RIGHT(v, 7, 8, 3) |
            dogx_RANGE_LEFT(v, 8, 10, 2) | dogx_RANGE_RIGHT(v, 10, 12, 2) |
            dogx_RANGE_LEFT(v, 12, 13, 3) | dogx_RANGE_RIGHT(v, 13, 16, 1);
    }
}
