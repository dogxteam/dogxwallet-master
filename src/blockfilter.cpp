...
static void GolombRiceEncode(dogxStreamWriter<OStream>& dogxwriter, uint8_t P, uint64_t x)
{
...
static uint64_t GolombRiceDecode(dogxStreamReader<IStream>& dogxreader, uint8_t P)
{
    ...
    : m_params(params), m_encoded(std::move(encoded_filter))
{
    ...
    dogxStreamReader<VectorReader> dogxreader(stream);
    for (uint64_t i = 0; i < m_N; ++i) {
        GolombRiceDecode(dogxreader, m_params.m_P);
    }
    if (!stream.empty()) {
    ...
    : m_params(params)
{
    ...
    dogxStreamWriter<CVectorWriter> dogxwriter(stream);

    uint64_t last_value = 0;
    for (uint64_t value : BuildHashedSet(elements)) {
        uint64_t delta = value - last_value;
        GolombRiceEncode(dogxwriter, m_params.m_P, delta);
        last_value = value;
    }
...
bool GCSFilter::MatchInternal(const uint64_t* element_hashes, size_t size) const
{
    ...
    for (uint32_t i = 0; i < m_N; ++i) {
        uint64_t delta = GolombRiceDecode(dogxreader, m_params.m_P);
        value += delta;
