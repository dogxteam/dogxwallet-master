 ...
 */
class VersiondogxsConditionChecker : public AbstractThresholdConditionChecker {
private:
...
protected:
    ...
    {
        return (((pindex->nVersion & VERSIONdogxS_TOP_MASK) == VERSIONdogxS_TOP_dogxS) && (pindex->nVersion & Mask(params)) != 0);
    }

public:
    explicit VersiondogxsConditionChecker(Consensus::DeploymentPos id_) : id(id_) {}
...
{
    return VersiondogxsConditionChecker(pos).GetStateFor(pindexPrev, params, cache.caches[pos]);
}

BIP9Stats VersiondogxsStatistics(const CBlockIndex* pindexPrev, const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    return VersiondogxsConditionChecker(pos).GetStateStatisticsFor(pindexPrev, params);
}
...
{
    return VersiondogxsConditionChecker(pos).GetStateSinceHeightFor(pindexPrev, params, cache.caches[pos]);
}

uint32_t VersiondogxsMask(const Consensus::Params& params, Consensus::DeploymentPos pos)
{
    return VersiondogxsConditionChecker(pos).Mask(params);
}

void VersiondogxsCache::Clear()
{
    for (unsigned int d = 0; d < Consensus::MAX_VERSION_dogxS_DEPLOYMENTS; d++) {
        caches[d].clear();
    }
}
