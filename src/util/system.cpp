...
const char * const dogxCOIN_CONF_FILENAME = "dogxcoin.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\dogxcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\dogxcoin
    // Mac: ~/Library/Application Support/dogxcoin
    // Unix: ~/.dogxcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "dogxcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/dogxcoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", dogxCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure dogxcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("dogxcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The dogxcoin Core developers";
    }
