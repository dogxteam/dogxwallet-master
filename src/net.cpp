...
void AdvertiseLocal(CNode *pnode)
{
    if (fListen && pnode->fSuccessfullyConnected)
    {
        ...
        if (IsPeerAddrLocalGood(pnode) && (!addrLocal.IsRoutable() ||
             rng.randdogxs((GetnScore(addrLocal) > LOCAL_MANUAL) ? 3 : 1) == 0))
        {
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (dogxcoin users should be well used to small numbers with many decimal places by now :)
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "dogxcoin " + FormatFullVersion();
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            ServiceFlags requiredServicedogxs = GetDesirableServiceFlags(NODE_NONE);
            std::string host = strprintf("x%x.%s", requiredServicedogxs, seed);
            ...
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true))
            {
                ...
                {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip, Params().GetDefaultPort()), requiredServicedogxs);
