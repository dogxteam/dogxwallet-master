...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a dogxcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new dogxcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new dogxcoinAddressCheckValidator(parent));
}

bool parsedogxcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!dogxcoinUnits::parse(dogxcoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsedogxcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsedogxcoinURI(uriInstance, out);
}

QString formatdogxcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(dogxcoinUnits::format(dogxcoinUnits::BTC, info.amount, false, dogxcoinUnits::separatorNever));
        paramCount++;
    }
...
bool opendogxcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", dogxCOIN_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "dogxcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "dogxcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("dogxcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for dogxcoin*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=dogxcoin\n";
        else
            optionFile << strprintf("Name=dogxcoin (%s)\n", chain);
...
bool GetStartOnSystemStartup()
{
    ...
    bool res = (findStartupItemInList(listSnapshot, loginItems, dogxcoinAppUrl) != nullptr);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    CFArrayRef listSnapshot = LSSharedFileListCopySnapshot(loginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(listSnapshot, loginItems, dogxcoinAppUrl);
