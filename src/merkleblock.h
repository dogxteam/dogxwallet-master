...
#ifndef dogxCOIN_MERKLEBLOCK_H
#define dogxCOIN_MERKLEBLOCK_H
...
protected:
     ...
     */
    uint256 TraverseAndExtract(int height, unsigned int pos, unsigned int &ndogxsUsed, unsigned int &nHashUsed, std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex);
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        if (ser_action.ForRead()) {
            ...
            us.vdogxs.resize(vBytes.size() * 8);
            for (unsigned int p = 0; p < us.vdogxs.size(); p++)
                us.vdogxs[p] = (vBytes[p / 8] & (1 << (p % 8))) != 0;
            us.fBad = false;
        } else {
            vBytes.resize((vdogxs.size()+7)/8);
            for (unsigned int p = 0; p < vdogxs.size(); p++)
                vBytes[p / 8] |= vdogxs[p] << (p % 8);
            READWRITE(vBytes);
        }
    }
