...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    int nShift = (blockindex->ndogxs >> 24) & 0xff;
    double dDiff =
        (double)0x0000ffff / (double)(blockindex->ndogxs & 0x00ffffff);
...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("dogxs", strprintf("%08x", blockindex->ndogxs));
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("dogxs", strprintf("%08x", block.ndogxs));
    result.pushKV("difficulty", GetDifficulty(blockindex));
    result.pushKV("chainwork", blockindex->nChainWork.GetHex());
    result.pushKV("nTx", (uint64_t)blockindex->nTx);
...
{
    UniValue rv(UniValue::VOBJ);
    const ThresholdState thresholdState = VersiondogxsTipState(consensusParams, id);
    ...
    rv.pushKV("since", VersiondogxsTipStateSinceHeight(consensusParams, id));
    if (ThresholdState::STARTED == thresholdState)
    {
        UniValue statsUV(UniValue::VOBJ);
        BIP9Stats statsStruct = VersiondogxsTipStatistics(consensusParams, id);
...
static void BIP9SoftForkDescPushBack(UniValue& bip9_softforks, const Consensus::Params& consensusParams, Consensus::DeploymentPos id)
{
    ...
    if (consensusParams.vDeployments[id].nTimeout > 0)
        bip9_softforks.pushKV(VersiondogxsDeploymentInfo[id].name, BIP9SoftForkDesc(consensusParams, id));
}
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    ...
    for (int pos = Consensus::DEPLOYMENT_CSV; pos != Consensus::MAX_VERSION_dogxS_DEPLOYMENTS; ++pos) {
