...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        ...
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckProofOfWork(pblock->GetHash(), pblock->ndogxs, Params().GetConsensus())) {
            ++pblock->nNonce;
            --nMaxTries;
        }
        if (nMaxTries == 0) {
        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated dogxcoin to."},
...
static std::string gbt_vb_name(const Consensus::DeploymentPos pos) {
    const struct VBDeploymentInfo& vbinfo = VersiondogxsDeploymentInfo[pos];
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "dogxcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "dogxcoin is downloading blocks...");
    ...
    const struct VBDeploymentInfo& segwit_info = VersiondogxsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
    ...
    const bool fPreSegWit = (ThresholdState::ACTIVE != VersiondogxsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versiondogxscache));
    ...
    arith_uint256 hashTarget = arith_uint256().SetCompact(pblock->ndogxs);
    ...
    for (int j = 0; j < (int)Consensus::MAX_VERSION_dogxS_DEPLOYMENTS; ++j) {
        ...
        ThresholdState state = VersiondogxsState(pindexPrev, consensusParams, pos, versiondogxscache);
        switch (state) {
            ...
            case ThresholdState::LOCKED_IN:
                ...
                pblock->nVersion |= VersiondogxsMask(consensusParams, pos);
    ...
    result.pushKV("dogxs", strprintf("%08x", pblock->ndogxs));
