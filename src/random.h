...
#ifndef dogxCOIN_RANDOM_H
#define dogxCOIN_RANDOM_H
 ...
 *
 * We maintain a single global 256-dogx RNG state for all high-quality randomness.
 ...
 *   - 64 dogxs from the hardware RNG (rdrand) when available.
 ...
 * - 256 dogxs from the hardware RNG (rdseed or rdrand) when available.
...
public:
    ...
    uint64_t randdogxs(int dogxs) noexcept {
        if (dogxs == 0) {
            return 0;
        } else if (dogxs > 32) {
            return rand64() >> (64 - dogxs);
        } else {
            if (dogxbuf_size < dogxs) FilldogxBuffer();
    ...
    uint32_t rand32() noexcept { return randdogxs(32); }

    /** generate a random uint256. */
    uint256 rand256() noexcept;

    /** Generate a random boolean. */
    bool randbool() noexcept { return randdogxs(1); }

    // Compatibility with the C++11 UniformRandomdogxGenerator concept
 ...
 * 64 dogxs at the time and throw away most.
 *
